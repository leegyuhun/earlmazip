...
public interface ApiStatisticsRepository extends JpaRepository<AptInfo, Long> {
    ...
    @Query("select a from ApiCallStat a where a.callDate = :callDate and a.apiGubn <> 'MENU' order by a.cnt desc")
    List<ApiCallStat> findAllToday(@Param("callDate") String callDate);

    @Query("select a from ApiCallStat a where a.callDate = :callDate and a.apiGubn = :apiGubn and a.apiGubn <> 'MENU' order by a.cnt desc")
